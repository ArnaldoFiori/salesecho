-- ============================================================
-- SalesEcho v6 — 001_initial_schema.sql
-- Executar no Supabase SQL Editor (uma única vez)
-- Spec de referência: 02_DDL.md v2
-- ============================================================
...
-- 2.6 pending_sessions (bot Telegram — sem RLS)
...
-- 4.2 Trigger de onboarding (pós-signup)
...
BEGIN
    -- Criar organization
    INSERT INTO public.organizations (name, estimated_sellers)
    VALUES (
        NEW.raw_user_meta_data->>'company_name',
        COALESCE((NEW.raw_user_meta_data->>'estimated_sellers')::INTEGER, 1)
    )
    RETURNING id INTO new_org_id;

    -- Criar user (org_admin)
    INSERT INTO public.users (auth_user_id, org_id, name, email, role, job_title)
    VALUES (
        NEW.id,
        new_org_id,
        COALESCE(NEW.raw_user_meta_data->>'full_name', 'Gestor'),
        NEW.email,
        'org_admin',
        NEW.raw_user_meta_data->>'job_title'
    );

    -- Criar subscription (trial 30 dias, limite 5 sellers)
    INSERT INTO public.subscriptions (org_id, status, trial_ends_at, seller_limit)
    VALUES (
        new_org_id,
        'trial',
...
-- Executar após rodar o script para confirmar:
-- SELECT table_name FROM information_schema.tables
-- WHERE table_schema = 'public' ORDER BY table_name;
--
-- Esperado (7 tabelas):
-- customers, organizations, pending_sessions, recordings,
-- subscriptions, support_audit, users
